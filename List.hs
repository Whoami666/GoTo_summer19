...
headOr z Nil = z
headOr _ (h:._) = h 
-- (!!) listik n -- берет н-ный элемент из списка listik
...
filter b Nil = Nil
filter b (x :. xs) = if b x == False then filter b (xs) else x :. filter b (xs) 
...
--
-- prop> \x -> flatetMap id (x :: List (List Int)) == flatten x
...
find _ Nil = Empty
find f (x :. xs) = if f x == True then (Full x) else find f xs
